...
export class StatudService {
  ...
  constructor(@InjectRepository(Statud) private statudServices: Repository<Statud>,
  ...
  async create(createStatudDto: CreateStatudDto) {
   ...
   if(!newStatud) return new HttpException('ERROR_NEW-STATUD_FOUND', HttpStatus.FOUND);
  ...
  async findOne(id: number) {
    ...
    const result = await this.statudServices.findOne({where:{id}, relations:['details']});

    if(!result) return new HttpException('ERROR_STATUD_NOT_FOUND', HttpStatus.NOT_FOUND);

    return result;
  }

  async getStatudForUser(userId:number) {
    const result = await this.statudServices.find({where:{userId},relations:['details']});
    if(!result) return new HttpException('ERROR_STATUD_NOT_FOUND', HttpStatus.NOT_FOUND);
    return result;
  }
  ...
  async update(id: number, updateStatudDto: UpdateStatudDto) {
    ...
    if(statudFound)
    {
      const updateStatud = await this.statudServices.update(id, updateStatudDto);
      if( updateStatud.affected == 1) 
